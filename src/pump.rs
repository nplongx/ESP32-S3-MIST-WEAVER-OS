...
pub struct PumpController {
    ...
    // Thay thế van và bơm chính bằng 2 bơm độc lập
    water_pump_in: LedcDriver<'static>,
    water_pump_out: LedcDriver<'static>,
...
impl PumpController {
    ...
    pub fn new(
        ...
        mut water_pump_in: LedcDriver<'static>,
        mut water_pump_out: LedcDriver<'static>,
        ...
        water_pump_in.set_duty(0)?;
        water_pump_out.set_duty(0)?;

        osaka_en.set_low()?;
        osaka_rpwm.set_duty(0)?;
        // osaka_lpwm.set_duty(0)?;

        info!("✅ Đã khởi tạo PumpController (Sử dụng 2 bơm nước vào/ra độc lập).");
    ...
    pub fn set_water_pump(&mut self, direction: WaterDirection) -> anyhow::Result<()> {
        let max_pump_in = self.water_pump_in.get_max_duty();
        let max_pump_out = self.water_pump_out.get_max_duty();

        match direction {
            WaterDirection::In => {
                // Tắt bơm ra trước để đảm bảo an toàn, sau đó bật bơm vào
                self.water_pump_out.set_duty(0)?;
                thread::sleep(Duration::from_millis(100)); // Delay nhỏ tránh sụt áp đột ngột
                self.water_pump_in.set_duty(max_pump_in)?;
            }
            WaterDirection::Out => {
                // Tắt bơm vào trước, sau đó bật bơm ra
                self.water_pump_in.set_duty(0)?;
                thread::sleep(Duration::from_millis(100));
                self.water_pump_out.set_duty(max_pump_out)?;
            }
            WaterDirection::Stop => {
                // Tắt cả hai bơm
                self.water_pump_in.set_duty(0)?;
